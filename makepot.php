...
class MakePOT {
	...
	public $meta = array(
		...
		'wp15' => array(
			'description' => 'wp15.wordpress.net',
			'copyright-holder' => 'WordPress',
			'package-name' => 'wp15.wordpress.net',
		),
		'rosetta' => array(
	...
	public function wordcamporg( $dir, $output ) {
		...
		/*
		 * WordCamp.org's plugin and theme directories are fully open-source, and therefore contain `svn:externals`
		 * for 3rd-party code (rather than those residing in the private WordCamp.org repository). 3rd-party
		 * strings should not be included in the project, though.
		 *
		 * New custom code that's added to the repository will need to be added here, unless it's named with the
		 * standard `wordcamp-` prefix.
		 */
		$includes = array(
			'mu-plugins/.*',

			'plugins/bbpress-network-templates/.*',
			'plugins/camptix-admin-flags/.*',
			'plugins/camptix-attendance/.*',
			'plugins/camptix-badge-generator/.*',
			'plugins/camptix-mailchimp/.*',
			'plugins/email-post-changes-specific-post/.*',
			'plugins/jquery-ui-css/.*',
			'plugins/multi-event-sponsors/.*',
			'plugins/wc-canonical-years/.*',
			'plugins/wc-fonts/.*',
			'plugins/wc-post-types/.*',
			'plugins/wcpt/.*',
			'plugins/wordcamp-.*/.*',

			'themes/campsite-2017/.*',
			'themes/plan/.*',
			'themes/wordcamp-.*/.*',
		);

		return $this->xgettext( 'wordcamporg', $dir, $output, array(), array(), $includes );
	}

	public function wp15( $dir, $output ) {
		$output = is_null( $output ) ? 'wp15.pot' : $output;

		$includes = array(
			'themes/twentyseventeen-wp15/.*',
		);

		return $this->xgettext( 'wp15', $dir, $output, array(), array(), $includes );
	}
